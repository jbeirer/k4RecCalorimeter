...
CreateODDCells::CreateODDCells(const std::string& aName, ISvcLocator* aSL) : base_class(aName, aSL) {
  ...
  for (uint iSys = 0; iSys < m_readoutNames.size(); iSys++) {
    ...
    while ((pos = tmp.find(delimiter)) != std::string::npos) {
      ...
      if (numVolumes.back() == 0) {
        error() << "Volume name " << m_activeVolumeNames[iSys] << " not found! Check naming in detector description."
                << endmsg;
        return StatusCode::FAILURE;
      }
    ...
    for (int iVol = numVolumes.size() - 1; iVol > 0; iVol--) {
      if (iVol > 0)
        numVolumes[iVol] = numVolumes[iVol] / numVolumes[iVol - 1];
      info() << "Number of active volumes named " << nameVolumes[iVol] << " within " << nameVolumes[iVol - 1] << " is "
             << numVolumes[iVol] << endmsg;
    ...
    if (msgLevel() == MSG::DEBUG) {
      ...
      myfile << "cellId, x, y, z, phi, R, dphi, dR, dz, xA, yA, zA, xB, yB, zB, xC, yC, zC, xD, yD, zD, dx, dy"
             << std::endl;
    }
    ...
    for (unsigned int imodule = 0; imodule < numVolumes[1]; imodule++) {
      for (unsigned int ilayer = 0; ilayer < numVolumes[numVolumes.size() - 2]; ilayer++) {
        ...
        if (!ifXzSegmentation) {
          ...
          auto numCells =
              numberOfCells(volumeId, *dynamic_cast<dd4hep::DDSegmentation::CartesianGridXY*>(segmentation));
          ...
          for (unsigned int ix = 0; ix < numCells[0]; ix++) {
            for (unsigned int iy = 0; iy < numCells[1]; iy++) {
              ...
              double inLocalCentre[] = {gridOffsetX + (-0.5 * numCells[0] + ix + 0.5) * gridSizeX,
                                        gridOffsetY + (-0.5 * numCells[1] + iy + 0.5) * gridSizeY, 0};
              // point A, local x rotated in XY plane
              double inLocalEdgeX[] = {gridOffsetX + (-0.5 * numCells[0] + ix + 1.0) * gridSizeX,
                                       gridOffsetY + (-0.5 * numCells[1] + iy + 0.5) * gridSizeY, 0};
              // point B, local y becomes z
              double inLocalEdgeY[] = {gridOffsetX + (-0.5 * numCells[0] + ix + 0.5) * gridSizeX,
                                       gridOffsetY + (-0.5 * numCells[1] + iy + 1) * gridSizeY, 0};
              // point C, local Z rotated in XY plane
              double inLocalEdgeZ[] = {gridOffsetX + (-0.5 * numCells[0] + ix + 0.5) * gridSizeX,
                                       gridOffsetY + (-0.5 * numCells[1] + iy + 0.5) * gridSizeY, halfSizes[2]};
              // max point in XY plane: combination of A and C
              double inLocalEdgeXY1[] = {gridOffsetX + (-0.5 * numCells[0] + ix + 1) * gridSizeX,
                                         gridOffsetY + (-0.5 * numCells[1] + iy + 0.5) * gridSizeY, halfSizes[2]};
              // max point in XY plane: combination of A and C
              double inLocalEdgeXY2[] = {gridOffsetX + (-0.5 * numCells[0] + ix + 1) * gridSizeX,
                                         gridOffsetY + (-0.5 * numCells[1] + iy + 0.5) * gridSizeY, -halfSizes[2]};
              ...
              double outGlobalCentre[3], outGlobalEdgeX[3], outGlobalEdgeY[3], outGlobalEdgeZ[3], outGlobalEdgeXY1[3],
                  outGlobalEdgeXY2[3];
              ...
              CLHEP::Hep3Vector edgePhiLow(outGlobalEdgeX[0], outGlobalEdgeX[1],
                                           outGlobalEdgeX[2]); // (x+xa,y+ya), (x-xa,y-ya)
              CLHEP::Hep3Vector edgePhiHigh(outGlobalEdgeX[0] + outGlobalCentre[0] - outGlobalEdgeZ[0],
                                            outGlobalEdgeX[1] + outGlobalCentre[1] - outGlobalEdgeZ[1],
                                            outGlobalEdgeX[2] + outGlobalCentre[2] - outGlobalEdgeZ[2]);
              CLHEP::Hep3Vector edgeRLow(outGlobalEdgeZ[0], outGlobalEdgeZ[1],
                                         outGlobalEdgeZ[2]); // (x+xc,y+yc), (x-xc,y-yc)
              CLHEP::Hep3Vector edgeRHigh(outGlobalEdgeZ[0] + outGlobalCentre[0] - outGlobalEdgeX[0],
                                          outGlobalEdgeZ[1] + outGlobalCentre[1] - outGlobalEdgeX[1],
                                          outGlobalEdgeZ[2] + outGlobalCentre[2] - outGlobalEdgeX[2]);
              ...
              // write to ROOT file (from cm to mm)
              eta = centre.eta();
              phi = centre.phi();
              r = centre.rho() * 10;
              ...
              if (msgLevel() == MSG::DEBUG) {
                ...
                /// cellID, xyz, phi r
                myfile << volumeId << ", " << x << ", " << y << ", " << z << ", " << phi << ", " << r << ", " << dphi
                       << ", " << dr << ", " << dz
                       << ", "
                       /// point A - centre
          ...
          // Get number of segmentation cells
          auto numCells =
              numberOfCells(volumeId, *dynamic_cast<dd4hep::DDSegmentation::CartesianGridXZ*>(segmentation));
          ...
          for (unsigned int iz = 0; iz < numCellsZ; iz++) {
            ...
            for (unsigned int ix = 0; ix < numCells[iz]; ix++) {
              // first assume an identical width for the other cells.
              double inLocalCentre[] = {gridOffsetX + (-0.5 * numCells[iz] + ix + 0.5) * gridSizeX, 0,
                                        gridOffsetY + (-0.5 * numCellsZ + iz + 0.5) * gridSizeY};
              // point A, local x rotated in XY plane
              double inLocalEdgeX[] = {gridOffsetX + (-0.5 * numCells[iz] + ix + 1.0) * gridSizeX, 0,
                                       gridOffsetY + (-0.5 * numCellsZ + iz + 0.5) * gridSizeY};
              // point B, local y rotated in XY plane
              double inLocalEdgeY[] = {gridOffsetX + (-0.5 * numCells[iz] + ix + 0.5) * gridSizeX, 0,
                                       gridOffsetY + (-0.5 * numCellsZ + iz + 1) * gridSizeY};
              // point C, local Z remains in Z
              double inLocalEdgeZ[] = {gridOffsetX + (-0.5 * numCells[iz] + ix + 0.5) * gridSizeX, halfSizes[2],
                                       gridOffsetY + (-0.5 * numCellsZ + iz + 0.5) * gridSizeY};
              // max point in XY plane: combination of A and C
              double inLocalEdgeXY1[] = {gridOffsetX + (-0.5 * numCells[iz] + ix + 1) * gridSizeX, halfSizes[2],
                                         gridOffsetY + (-0.5 * numCellsZ + iz + 0.5) * gridSizeY};
              // max point in XY plane: combination of A and C
              double inLocalEdgeXY2[] = {gridOffsetX + (-0.5 * numCells[iz] + ix + 1) * gridSizeX, -halfSizes[2],
                                         gridOffsetY + (-0.5 * numCellsZ + iz + 0.5) * gridSizeY};
              ...
              CLHEP::Hep3Vector edgePhiHigh(outGlobalEdgeX[0] + outGlobalCentre[0] - outGlobalEdgeZ[0],
                                            outGlobalEdgeX[1] + outGlobalCentre[1] - outGlobalEdgeZ[1],
                                            outGlobalEdgeX[2] + outGlobalCentre[2] - outGlobalEdgeZ[2]);
              CLHEP::Hep3Vector edgeRLow(outGlobalEdgeY[0], outGlobalEdgeY[1],
                                         outGlobalEdgeY[2]); // (x+xc,y+yc), (x-xc,y-yc)
              CLHEP::Hep3Vector edgeRHigh(outGlobalEdgeY[0] + outGlobalCentre[0] - outGlobalEdgeX[0],
                                          outGlobalEdgeY[1] + outGlobalCentre[1] - outGlobalEdgeX[1],
                                          outGlobalEdgeY[2] + outGlobalCentre[2] - outGlobalEdgeX[2]);
              // set other fields to retrieve the correct cellID of the active volume
              decoder->set(volumeId, "slice", idOfSlice);
              decoder->set(volumeId, "x", firstCellX + int(ix));
